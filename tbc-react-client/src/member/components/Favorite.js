...
const Favorite = ({ userId, projectId }) => {
    ...
    console.log('Favorite: login-userId=', userId, ', projectId=', projectId)
		Pas.getUser(userId)
			.then(res => {
				setLike(res.data.likeProjects.split(',').filter(like=>like == projectId).length !== 0); // 숫자와 문자열 비교라서 '==' 사용.
			})
	...
	const onToggle = () => {
        ...
        let member = res.data;
				if (like) // 이미 있으니 지운다.
					member.likeProjects = member.likeProjects.split(',').filter(like => like != projectId).toString();
				else { // 없으니 추가한다.
